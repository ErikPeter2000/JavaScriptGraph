...
export class GraphViewer {
  ...
  drawGraph() {
    ...
    this.graph = ForceDirectedGraphCalculator.repositionNodes(
      this.graph,
      this.canvas.width,
      this.canvas.height,
      10,
  ...
  addArc(idFrom, idTo) {
    let arc = new ArcObject({ idFrom, idTo });
  ...
  clickSelection() {
    let found = false;
    let relativePos = this.cursorState.mousePos
      .sub(this.viewPreview)
      .div(this.viewScale);
    if (this.cursorState.selectMode & SelectMode.NODE) {
      for (let nodeId in this.graph.nodeData) {
        if (
          this.graph.nodeData[nodeId].isClicked(relativePos) &&
          !this.selection.nodes.includes(nodeId)
        ) {
      ...
      this.selection.nodes = [];
      {
        if (this.cursorState.selectMode & SelectMode.ARC) {
          for (let arc of this.graph.arcData) {
            ...
            if (
              arc.isClicked(relativePos, this.graph) &&
              !this.selection.arcs.includes(id)
            ) {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      for (let nodeId in graph.nodeData) {
        ...
        for (let otherId of graph.getNeighbours(nodeId)) {
          ...
          force = force.add(
            delta.normalise().mul(this.#forceConstant * distance),
          );
        }
        ...
        for (let otherId in graph.nodeData) {
          ...
          force = force.sub(
            delta.mul(
              ((this.#idealDistance * this.#idealDistance) /
                distance /
                distance) *
